...
parser.add_argument('--dist-backend', default='cncl', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not torch.mlu.is_available():
        ...
        if args.arch.startswith('alexnet') or args.arch.startswith('vgg'):
            ...
            model = torch.nn.DataParallel(model).mlu()

    # define loss function (criterion), optimizer, and learning rate scheduler
    criterion = nn.CrossEntropyLoss().mlu(args.gpu)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            images = images.mlu(args.gpu, non_blocking=True)
        if torch.mlu.is_available():
            target = target.mlu(args.gpu, non_blocking=True)
...
def validate(val_loader, model, criterion, args):
    ...
    print("jit trace done.")

    #compile magicmind model
    inputs = [torch_mlu.Input(
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            acc1, acc5 = accuracy(output[0:bs], target[0:bs], topk=(1, 5))
